...
def start_hotspot():
    """Start wifi-connect captive portal to collect home WiFi credentials."""
    print("\n[Stage A] Starting hotspot...")
    CREDENTIALS_FILE.parent.mkdir(parents=True, exist_ok=True)

    # wifi-connect writes credentials to stdout when user submits the form
    # We capture them and save to file.
    result = run(
        f"wifi-connect --portal-ssid '{PORTAL_SSID}' "
        f"--portal-interface {WIFI_IFACE} "
        f"--output {CREDENTIALS_FILE}"
    )
