...
The Activation of Flexible Resources enabler (EN_35) is the action layer of UC4's operational architecture. It translates flexibility needs — identified by EN_36 or requested by the DSO — into validated, safe, and explainable actuation commands dispatched to the community's flexible resources. As specified in D4.2, EN_35 targets battery energy storage systems and electric vehicle charging infrastructure as its primary actuatable assets, and its design prioritises low-latency, edge-local communication that enables autonomous dispatch without dependence on centralised cloud services. The enabler acquires real-time status from a heterogeneous set of participating IoT devices (REQ_UC4.10), combines this with EN_33 pre-act simulation and EN_29 confidence gating, and produces actuation decisions that are traceable, safe, and auditable through XAI-generated explanations and EN_25 human-in-the-loop validation.

EN_35 directly addresses IoT data retrieval and actuation of IoT devices (REQ_UC4.10) and DSO emergency flexibility signal for real-time direct load control (REQ_UC4.12). It is the operational realisation of all three UC4 flexibility scenarios: battery discharge and EV charge modulation for voltage support, load reduction for line capacity management, and energy self-sufficiency during high price periods.
...
flowchart LR
    ...
    EVSE -->|current state| SG
    EVSE -->|current state| FC
    DT   -->|simulation result| PS
    GL   -->|confidence gate| FC
    AE   -->|discharge command| BAT
    AE   -->|charge modulation| EV
    XG   -->|why · what · expected effect| DASH
    XG   -->|explanation + impact| HITL
    AE   -->|record| LOG

    class VM,OP,BMS,EVSE,DT,GL src
    class SG guard
    class FC,PS,AE,XG proc
    class BAT,EV,DASH,HITL,LOG dst
```

The Safeguards module checks operational constraints for each resource type before any action is evaluated. For the battery bank: minimum state of charge (SoC floor, default 20%), maximum discharge rate, cooldown period since last activation (default 15 minutes), and availability status reported by the BMS. For EV charging stations: the owner's declared departure time and minimum departure SoC, ensuring that no flexibility activation compromises the vehicle's readiness for its next trip. Requests that violate any constraint are immediately rejected with an explanatory response published back to the `flexibility-requests` topic.

The Feasibility Check evaluates whether the requested flexibility magnitude is achievable given the current state of available resources — battery bank and enrolled EV chargers — and the contracted flexibility parameters for the affected community members. When multiple resource types can satisfy a request, the Feasibility Check selects the combination that best matches the required response time and available headroom. It also invokes EN_29's confidence gate: if the current community confidence score S < 0.6, the action is escalated to EN_25 for mandatory human validation regardless of the triggering event's CRITICAL classification.

The Pre-act Simulation submits the proposed action to EN_33 as a hypothetical scenario: EN_33 runs a power flow calculation with the selected activations applied and returns the projected network state. If the simulation confirms that the action resolves the triggering issue without creating new constraint violations (e.g., causing undervoltage on another bus), the action proceeds. CRITICAL-level automated actions proceed directly to the Actuation Engine; WARNING-level actions and all actions flagged by EN_29's confidence gate are routed to EN_25 for operator approval before execution.

The Actuation Engine dispatches validated commands to the target devices via direct, low-latency control interfaces over the local network, bypassing the Kafka message bus for command dispatch. This ensures that the critical control loop operates within the edge network boundary without requiring cloud connectivity — a key design principle carried forward from D4.2's specification of resilient, decentralised microgrid management. The Actuation Engine monitors device responses (command acknowledged, power output ramping, SoC trajectory) and publishes state updates back to EN_33's State Updater to keep the network model current.
...
The Activation Record persists the full event — trigger, decision rationale, EN_29 confidence score at time of decision, simulation result, operator approval (if EN_25 path was taken), per-device commands and responses, and post-activation metered outcome — to PostgreSQL for audit, EN_26 reporting, and model improvement.

The Safeguards, Feasibility Check, and Pre-act Simulation components are implemented in Python as a stateful microservice deployed on the GECAD edge Kubernetes cluster. The Actuation Engine communicates with the battery BMS and EV charging stations via low-latency local network interfaces, reflecting D4.2's R&D focus on communication protocols that enable autonomous actuation without centralised cloud intervention. The co-location of EN_35 with EN_33 and EN_36 on the edge cluster ensures that the full perception-decision-actuation loop remains operational even during external connectivity disruptions. The XAI Generator uses a template-based explanation framework combined with SHAP-based feature importance attribution for the flexibility estimation model outputs. Access is restricted to authorised platform services via Kubernetes network policy. Activation records are signed with a service identity key to support non-repudiation (REQ_UC4.15).

EN_35 is where UC4's intelligence becomes physical action. By implementing a rigorous pre-actuation validation pipeline — including device-aware safety checks, EN_33 simulation-based verification, EN_25 human approval (where required), EN_29 confidence gating, and XAI-generated rationale — it ensures that the system's decisions are both effective and trustworthy. The combination of battery storage activation, EV charge modulation, and EN_32 load control at member premises gives UC4 a comprehensive flexibility portfolio spanning the heterogeneous IoT device landscape specified in D4.2, capable of addressing all three operational scenarios. The edge-local deployment ensures resilient operation independent of centralised cloud infrastructure. EN_35 is the primary driver of the KPI targets for voltage issue frequency and resolution time.
