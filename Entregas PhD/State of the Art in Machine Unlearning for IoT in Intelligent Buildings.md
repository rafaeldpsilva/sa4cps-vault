...
IB-IoT systems generate four broad categories of sensitive data, each carrying its own privacy profile. **Occupancy traces** — presence/absence flags, room-level localisation, dwell time, and movement paths inferred from PIR sensors, BLE beacons, Wi-Fi probe requests, or camera analytics — reveal occupant routines, social ties, and work patterns with a fidelity unmatched by traditional surveillance [[50]]. **Energy and consumption profiles**, captured at per-circuit or per-appliance resolution by smart meters, are equally revealing: non-intrusive load monitoring (NILM) techniques can disaggregate aggregate consumption into individual device usage and from there infer activities such as cooking, sleeping, or showering [[51]]. **Environmental and biometric signals** — CO₂, temperature, and humidity readings that act as proxies for occupant count, alongside wearable telemetry and access-control biometrics such as fingerprint, face, and gait — sit at the boundary between ambient sensing and direct identification [[52]]. Finally, the **derived inferences** that ML pipelines extract from these raw streams — comfort preferences, schedules, health proxies — are frequently more sensitive than the inputs because they aggregate across modalities and surface latent attributes the occupant never disclosed [[53]]. All four categories qualify as personal data under GDPR once tied to an identifiable occupant, and the threshold for identifiability is low: a device MAC address, a lease record, or a behavioural fingerprint is sufficient to bring them within scope [[13]].

The regulatory regime applicable to these categories has three load-bearing components. **GDPR Article 5** establishes the principles of data minimisation, purpose limitation, and storage limitation; **Article 17** codifies the right to erasure; and **Article 25** imposes data protection by design and by default — together defining the EU baseline against which any IB-IoT deployment is measured [[13]]. The **ePrivacy Directive** complements the GDPR by governing electronic communications metadata, which is directly relevant to occupancy data carried over building networks and to device-to-device telemetry [[54]]. Common data-minimisation practices — edge aggregation that replaces raw video with occupancy counts, differential-privacy noise applied to telemetry, and bounded retention windows — reduce raw data exposure but do nothing to remove statistical traces already absorbed into trained models [[55]]. Consent in multi-tenant buildings further complicates this picture: it must be **granted** per-tenant and per-purpose at lease start, **updated** on policy revision or sub-letting, and **revoked** on move-out, dispute, or unilateral objection. Shared spaces such as lobbies and gyms create **overlapping consent domains** in which one occupant's revocation cannot retroactively unmix their contribution from co-occupant data captured by the same sensor [[56]]. Layered on top of this is a **data-controller ambiguity** problem: building owner, facility manager, vendor cloud, and tenant may each act as controller for different streams, leaving it unclear who is technically and legally responsible for executing a given unlearning request [[57]].

The consequence for machine unlearning is that IB-IoT deployments face high-throughput, multi-source revocations rather than the rare, well-formed deletion requests assumed in §2. Tenant turnover, short-term occupancy, contract workers, and visitors collectively make revocations a continuous arrival process rather than a batch event, and any unlearning pipeline must keep pace with the rate at which occupants change — orders of magnitude above the deletion frequency for which existing methods were designed. Each request can span multiple data subjects' overlapping records, so the unit of unlearning is rarely a clean single-sample slice; methods that assume sample-level granularity must be extended to handle entangled co-occupant contributions. Finally, derived inferences must be **transitively unlearned**: erasing raw occupancy data does not by itself remove a downstream comfort-preference or anomaly-detection model trained on it, and the dependency graph between raw streams and derived models is rarely tracked end-to-end in current deployments [[58]]. These three properties — high arrival rate, multi-subject entanglement, and transitive scope — set the throughput and granularity requirements that §4 measures existing methods against.
