...
The consequence of this architecture for machine unlearning is structural rather than incidental. Model state is **replicated across tiers**: the cloud holds the trained reference model, the edge runs a distilled or quantised derivative, and the perception tier may carry a further-reduced threshold variant tuned to its sensing modality. A single deletion request therefore propagates not to one artefact but to every tier holding a derived copy, and demands a coordination protocol that the centralised settings assumed in §2 simply did not require. Worse, the lossy transforms — quantisation, distillation, pruning — that produce edge replicas sever the **provenance link** between individual training samples and the parameters of the deployed model; even certified-removal procedures such as SISA [[22]] cannot retroactively reach an edge replica whose weights are no longer in one-to-one correspondence with the cloud-side training shards [[49]]. Finally, intermittent connectivity blocks synchronous propagation of revocations, so the system must tolerate **partial-unlearn states** in which some tiers have forgotten while others have not — a failure mode that no unlearning definition surveyed in §2.2 explicitly accommodates. These three properties — replicated state, broken provenance, and asynchronous propagation — are the architectural reasons why IB-IoT unlearning cannot be obtained by transplanting centralised methods, and they recur as structural constraints in the analysis of §4.2.
